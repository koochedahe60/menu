...
(function(){
  ...
  async function buildOverlay(){
    ...
    const androidSteps = `
      <div style="text-align:center;line-height:2.1;font-size:.92rem;color:var(--ink,#241608)">
        <p>با زدن دکمهٔ زیر، کوچه دهه شصت مستقیم به صفحه اصلی گوشی شما اضافه می‌شود.</p>
      </div>
      <button type="button" id="pibo-install-btn" class="btn btn-primary" style="width:100%;margin-top:6px">نصب اپلیکیشن کوچه دهه شصت</button>
    `;
    ...
    overlay.innerHTML = `
      <div style="width:100%;max-width:460px;background:var(--white,#FFFDF8);border-radius:28px 28px 0 0;padding:30px 24px 26px;box-shadow:0 -10px 40px rgba(0,0,0,.25);animation:pibo-slide-up .35s var(--ease,ease) both">
        ...
        <h2 style="text-align:center;font-size:1.15rem;font-weight:900;margin:0 0 6px;color:var(--ink,#241608)">نصب اپلیکیشن وب کوچه دهه شصت</h2>
